...
from network.attention import TemporalAttentionModule, ChannelAttentionModule
...
class MFNET_3D(nn.Module):
    ...
    def forward(self, x):
        ...
        h2 = self.conv2(h)   #  x56 ->  x56
        h2_cam = self.cam(h2)
        h2_tam = self.tam(h2)
        h = h2_cam + h2_tam

        h3 = self.conv3(h)   #  x56 ->  x28
        h3_cam = self.cam(h3)
        h3_tam = self.tam(h3)
        h = h3_cam + h3_tam

        h4 = self.conv4(h)   #  x28 ->  x14
        h4_cam = self.cam(h4)
        h4_tam = self.tam(h4)
        h = h4_cam + h4_tam

        h5 = self.conv5(h)   #  x14 ->   x7
        h5_cam = self.cam(h5)
        h5_tam = self.tam(h5)
        h = h5_cam + h5_tam
