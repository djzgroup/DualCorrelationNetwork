...
class BN_AC_CONV3D(nn.Module):
    ...
    def __init__(self, num_in, num_filter,
        ...
        self.bn_sw = nn.BatchNorm3d(num_in)
        self.bn_of = nn.BatchNorm3d(num_in)
        self.relu = nn.ReLU(inplace=True)
        self.conv = nn.Conv3d(num_in, num_filter, kernel_size=kernel, padding=pad,
                               stride=stride, groups=g, bias=bias)
        self.conv_sw = nn.Conv3d(num_in, num_filter, kernel_size=kernel, padding=pad,
                              stride=stride, groups=g, bias=bias)
        self.conv_of = nn.Conv3d(num_in, num_filter, kernel_size=kernel, padding=pad,
                              stride=stride, groups=g, bias=bias)
        self.temporal_dropout = nn.Dropout(p=0.5, inplace=False)
        self.appearance_flow_dropout = nn.Dropout(p=0.75, inplace=False)

    def forward(self, x):
        rgb = x[0]
        of = x[1]
        h = self.relu(self.bn(rgb))
        h = self.conv(h)

        h_of = self.relu(self.bn_of(of))
        h_of = self.conv_of(h_of)

        h_sw = self.relu(self.bn_sw(of + rgb))
        h_sw = self.conv_sw(h_sw)

        h_sw = self.appearance_flow_dropout(h_sw)
        h_of = self.appearance_flow_dropout(h_of)
        h += h_sw
        h_of += h_sw
...
class MFNET_3D(nn.Module):
    ...
    def forward(self, x):
        ...
        rgb = self.conv1(rgb)   # x224 -> x112
        rgb = self.maxpool(rgb) # x112 ->  x56

        of = self.conv1(of)   # x224 -> x112
        of = self.maxpool(of) # x112 ->  x56
